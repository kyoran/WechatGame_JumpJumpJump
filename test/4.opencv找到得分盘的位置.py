...
####
# 寻找目标位置
def find_goal(img, chess_x, chess_y):
    # 遍历起点为分数下沿
    global chess_w, chess_h
    img[chess_x: chess_x + chess_w][chess_y: chess_y + chess_h] = 0

    board_y_top = under_game_score_y
    board_x_top = 0
    board_x_centers = []; board_x_center = 0;  # 跳板的中心
    ...
    for i in img[under_game_score_y:]:
        ...
        if calc_board_x_centers_count != 5:
            ...
            if len(np.extract(condition, i)) > 4:  # 避免图中无关的像素点干扰，大于4代表匹配成功
                ...
                print("左边开始：", end="")
                left_location = 0; right_location = i.shape[0]; # 左右两边的横坐标
                for left in i[0:]:
                    # 从左侧（0）到右边
                    left_location += 1
                    if left == 255:
                        break
                    # print(left, end=",")
                print("左边结束, left_location =", left_location)
                print("右边开始：", end="")
                for right in reversed(i[: i.shape[0]]):
                    ...
                    if right == 255:
                        break
                print("右边结束, right_location =", right_location)
            ...
            # 找y的中心点
            llocation = board_x_center; rlocation = board_x_center;
            up_space = down_space
            # 从跳板中心往左
            ...
            for right in i[board_x_center:]:
                ...
                if right == 255:
                    break
            # cv2.imshow("temp", img[board_y_center:])
            # cv2.waitKey(0)
            # 计算横坐标的跨度
            down_space = rlocation-llocation
            print("left location =", llocation)
            print("right location =", rlocation)
            print("up space =", up_space)
            print("down space =", down_space)
            if up_space-down_space>3:   # 误差是3
                break


    print("board_x_centers =", board_x_centers)
    print("board_x_center =", board_x_center)
    print("board_y_center =", board_y_center)
    # 画出顶点的坐标示意图
...
if __name__ == '__main__':
    ...
    game_img, goal_x, goal_y = find_goal(blurcanny(res_img), chess_x, chess_y)
